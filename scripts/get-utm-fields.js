...
const KOMMO_SUBDOMAIN = "gpuchkov";
...
async function fetchLeadFields() {
  try {
    ...
    if (data._embedded && data._embedded.custom_fields) {
      ...
      // Find UTM fields
      const utmFields = fields.filter((f) => f.name && f.name.toLowerCase().includes("utm"));
      ...
      if (utmFields.length > 0) {
        ...
        utmFields.forEach((field) => {
          ...
          console.log(`KOMMO_${envName}_FIELD_ID=${field.id}`);
        });
      } else {
        console.log("❌ No UTM fields found in lead custom fields");
      }
    } else {
